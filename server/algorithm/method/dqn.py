...
class Method(object):
    def __init__(self, agent, num_s, num_a, name, test, lr=0.0001, gamma=0.99, replace_target_iter=2000, memory_size=2000000,
                 batch_size=256, epsilon=0.1, epsilon_decay=0.0001):
        self.agent = agent
        self.name = name
        self.num_s = num_s
        self.num_a = num_a
    ...
    def act(self, state, avas, no_train):  # epsilon greedy
        if np.random.uniform() > self.epsilon or self.test or no_train:  # pick the argmax action
            ...
            if avas is not None:  # (full game mode)
                q_eval[avas == 0] = -float('inf')  # unavailable actions should never be selected !
            action = np.argmax(q_eval)
        else:  # pick random action
            action = np.random.randint(0, self.num_a) if avas is None else np.random.randint(0, sum(avas))
        return action
