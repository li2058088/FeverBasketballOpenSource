...
class Method(object):
    ...
    def __init__(self, agent, num_global_s, num_s, num_a, name, test, lr=0.0001, gamma=0.99, replace_target_iter=2000,
                 memory_size=2000000, batch_size=256, epsilon=1, epsilon_decay=0.0001, lambda_opt=0.2, lambda_nopt=0.2):
        self.agent = agent
        self.name = name
        ...
        qtot_params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=self.name + '/q_tot')
        qtot_t_params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=self.name + '/q_tot_target')

        with tf.variable_scope('soft_replacement'):
            self.target_replace_op = [tf.assign(t, e) for t, e in zip(t_params, e_params)]
            self.qtot_target_replace_op = [tf.assign(t, e) for t, e in zip(qtot_t_params, qtot_params)]
    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            with tf.variable_scope('eval_net'):
                ...
                self.q1_max = tf.reduce_max(self.q1_eval, axis=1, keep_dims=True)
                self.q1_sa = tf.reduce_sum(tf.multiply(self.q1_eval, tf.one_hot(self.a1, self.num_a)), axis=1,
                                           keep_dims=True)
                self.q1_argmax = tf.argmax(self.q1_eval, axis=1)
                ...
                self.q2_max = tf.reduce_max(self.q2_eval, axis=1, keep_dims=True)
                self.q2_sa = tf.reduce_sum(tf.multiply(self.q2_eval, tf.one_hot(self.a2, self.num_a)), axis=1,
                                           keep_dims=True)
                self.q2_argmax = tf.argmax(self.q2_eval, axis=1)
                ...
                self.q3_max = tf.reduce_max(self.q3_eval, axis=1, keep_dims=True)
                self.q3_sa = tf.reduce_sum(tf.multiply(self.q3_eval, tf.one_hot(self.a3, self.num_a)), axis=1,
                                           keep_dims=True)
                self.q3_argmax = tf.argmax(self.q3_eval, axis=1)
            ...
            # no mixing network is needed for constructing Q_tot
            ...
            with tf.variable_scope('q_tot'):
                qtot_input = tf.concat([self.S, tf.one_hot(self.a1, self.num_a), tf.one_hot(self.a2, self.num_a),
                                        tf.one_hot(self.a3, self.num_a)], axis=1)

                qtot_layer = tf.layers.dense(qtot_input, units=self.m_units, kernel_initializer=w_initializer,
                                             bias_initializer=b_initializer)
                qtot_layer = tf.nn.elu(qtot_layer, name='a1')
                self.Q_tot = tf.layers.dense(qtot_layer, units=1, kernel_initializer=w_initializer,
                                             bias_initializer=b_initializer, activation=tf.identity, name='q_tot_output')

            with tf.variable_scope('q_tot_target'):
                qtot_target_input = tf.concat([self.S_, tf.one_hot(self.q1_argmax, self.num_a),
                                               tf.one_hot(self.q2_argmax, self.num_a),
                                               tf.one_hot(self.q3_argmax, self.num_a)], axis=1)
                qtot_target_layer = tf.layers.dense(qtot_target_input, units=self.m_units, kernel_initializer=w_initializer,
                                             bias_initializer=b_initializer)
                qtot_target_layer = tf.nn.elu(qtot_target_layer, name='a1_')
                self.Q_tot_ = tf.layers.dense(qtot_target_layer, units=1, kernel_initializer=w_initializer,
                                             bias_initializer=b_initializer, activation=tf.identity,
                                              name='q_tot_target_output')

            # the definition of state-value network V
            with tf.variable_scope('state_value_net'):
                value_net_layer = tf.layers.dense(inputs=self.S, units=self.m_units, kernel_initializer=w_initializer,
                                             bias_initializer=b_initializer)
                value_net_layer = tf.nn.elu(value_net_layer, name='v_l1')
                value_net_layer = tf.layers.dense(inputs=value_net_layer, units=self.m_units, kernel_initializer=w_initializer,
                                                  bias_initializer=b_initializer)
                value_net_layer = tf.nn.elu(value_net_layer, name='v_l2')
                self.V = tf.layers.dense(inputs=value_net_layer, units=1, kernel_initializer=w_initializer,
                                         bias_initializer=b_initializer, activation=tf.identity,
                                         name='v_net')

            # the sum of each individual max q_i(si, ai), for computing opt loss
            with tf.variable_scope('sum_q_opt'):
                self.Q_sum_opt = self.q1_max + self.q2_max + self.q3_max

            # the sum of each individual q_i(si, ai), for computing nopt loss
            with tf.variable_scope('sum_q_nopt'):
                self.Q_sum_nopt = self.q1_sa + self.q2_sa + self.q3_sa

            # fixed Q_tot for computing opt and nopt losses
            with tf.variable_scope('hat_q_tot'):
                self.hat_q_tot_opt_phd = tf.placeholder(tf.float32, [None, 1], name='hat_Qtot_opt')
                self.hat_q_tot_nopt_phd = tf.placeholder(tf.float32, [None, 1], name='hat_Qtot_nopt')

            with tf.variable_scope('q_target'):
                q_target = self.R + (1 - self.done) * self.gamma * self.Q_tot_
                self.q_target = tf.stop_gradient(q_target)

            # loss of the true Q_tot function
            with tf.variable_scope('loss'):
                self.td_loss = tf.reduce_mean(tf.squared_difference(self.q_target, self.Q_tot, name='TD_error'))
                self.opt_loss = tf.reduce_mean(tf.squared_difference(self.Q_sum_opt + self.V, self.hat_q_tot_opt_phd),
                                           name='Opt_Loss')
                self.nopt_loss = tf.reduce_mean(tf.square(tf.minimum(self.Q_sum_nopt + self.V - self.hat_q_tot_nopt_phd, 0)),
                                                name='Nopt_Loss')
                self.loss = self.td_loss + self.opt_loss * self.lambda_opt + self.nopt_loss * self.lambda_nopt

            with tf.variable_scope('train'):
                self._train_op = tf.train.AdamOptimizer(self.lr).minimize(self.loss)  # org
    ...
    def update(self):
        ...
        a1_argmax, a2_argmax, a3_argmax = self.sess.run([self.q1_argmax, self.q2_argmax, self.q3_argmax],
                                                        feed_dict={
                                                            self.s1: s1,
                                                            self.s2: s2,
                                                            self.s3: s3
                                                        })
        hat_q_tot_opt_values = self.sess.run(self.Q_tot, feed_dict={
            self.S: S,
            self.a1: a1_argmax,
            self.a2: a2_argmax,
            self.a3: a3_argmax
        ...
        hat_q_tot_nopt_values = self.sess.run(self.Q_tot, feed_dict={
            self.S: S,
            self.a1: a1,
            self.a2: a2,
            self.a3: a3
        ...
        _, cost = self.sess.run([self._train_op, self.loss],
                                feed_dict={self.S: S, self.s1: s1, self.s2: s2, self.s3: s3,
                                           ...
                                           self.R: R,
                                           self.done: done,
                                           self.hat_q_tot_opt_phd: hat_q_tot_opt_values,
                                           self.hat_q_tot_nopt_phd: hat_q_tot_nopt_values
                                           #self.Q_tot_: q_tot_,
                                           })
        ...
        if self.learn_step_cnt % self.replace_target_iter == 0 and self.learn_step_cnt:
            ...
            self.sess.run(self.qtot_target_replace_op)
    ...
    # def _build_net_(self):  # we use parameter sharing among agents
    #     with tf.variable_scope(self.name):
    #         # ------------------ all inputs ------------------------
    #         self.S = tf.placeholder(tf.float32, [None, self.num_global_s], name='S')  # input Global State
