...
class Method(object):
    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            with tf.variable_scope('loss'):
                ...
                self.loss = tf.reduce_mean(tf.square(self.ws * self.td_error))
                self.ws_mean = tf.reduce_mean(self.ws)
    ...
    def update(self):
        ...
        _, cost, ws_mean = self.sess.run([self._train_op, self.loss, self.ws_mean],
        ...
        self.write_summary_scalar('weights', ws_mean, self.learn_step_cnt)
