#coding: utf-8
"""
The base class of AgentHandler, which is used to generate experience for single-agent perspective.
"""
...
class AgentHandler:
    ...
    def handle_info_packet(self, json_packet, scene=None):
        """
        ...
        # get the available actions to mask unavailable actions in full game mode
        available_actions = None if not self.full_game else self.get_available_actions_share(json_packet, scene,
                                                                                             ending_object=None)
        # get the local observations
        result_list = self.feature_extract(json_packet) if not self.full_game else \
        ...
        self.interface.SendSample(self.pre_result_list, self.last_action, reward, result_list, available_actions, False,
    ...
    def handle_ending_result(self, ending_object, now_state, scene=None):
        # get the available actions to mask unavailable actions in full game mode
        available_actions = None if not self.full_game else self.get_available_actions_share(now_state, scene,
                                                                                             ending_object=ending_object)
        # get the final reward
        reward = self.calc_final_reward(ending_object, now_state)
        if self.pre_result_list:
            self.interface.SendSample(self.pre_result_list, self.last_action, reward, self.pre_result_list,
                                      available_actions, True, no_train=self.should_fixed(now_state))
